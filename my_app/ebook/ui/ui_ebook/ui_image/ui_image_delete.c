 ...
 * @return {*}
 */
static void ui_image_delete_confirm_focus(lv_obj_t *obj, bool focus)
{
 ...
 * @return {*}
 */
static void ui_image_delete_confirm_key_event_cb(lv_event_t *e){
        ...
        switch(KEY){
            ...
            case LV_KEY_ENTER:
                switch(user_data){
                    case 0:
                        //隐藏弹窗,切换焦点组到图片显示的焦点组
                        lv_obj_add_flag(lv_obj_get_child(ui_image_display_list->ui_image_display, 1), LV_OBJ_FLAG_HIDDEN);
 ...
 * @return {*}
 */
static void ui_image_delete_confirm_group_set(lv_group_t *group,lv_obj_t *obj,int user_data){
    lv_group_add_obj(group, obj);
    lv_obj_add_event_cb(obj, ui_image_delete_confirm_focus_event_cb,LV_EVENT_FOCUSED , (void *)user_data);
    lv_obj_add_event_cb(obj, ui_image_delete_confirm_focus_event_cb,LV_EVENT_DEFOCUSED , (void *)user_data);
    lv_obj_add_event_cb(obj,ui_image_delete_confirm_key_event_cb,LV_EVENT_KEY, (void *)user_data);
}

/**********************************************初始化函数***********************************************/
void ui_image_delete_init(void){
    //1.创建一个弹窗容器
    ...
    //3.创建一个焦点组
    ui_image_display_list->ui_image_delete_group = lv_group_create();
     //4.创建两个确认容器
    for(int i = 0; i < 2; i++){
        delete_confirm_container[i] = lv_obj_create(pop_up);
        //设为聚焦的两个重要标志位
        lv_obj_add_flag(delete_confirm_container[i], LV_OBJ_FLAG_CLICKABLE);
        lv_obj_clear_flag(delete_confirm_container[i], LV_OBJ_FLAG_SCROLLABLE); 
        //设置按钮容器的大小和样式
        lv_obj_set_size(delete_confirm_container[i], 40, 32);
        lv_obj_set_style_border_color(delete_confirm_container[i], lv_color_black(), LV_PART_MAIN);
        lv_obj_set_style_border_width(delete_confirm_container[i], 1, LV_PART_MAIN);
        lv_obj_set_style_radius(delete_confirm_container[i], 10, LV_PART_MAIN);
        ...
        ui_image_delete_confirm_group_set(ui_image_display_list->ui_image_delete_group,delete_confirm_container[i],i);
    }
    //5.默认聚焦第一个【否】
    lv_group_focus_obj(delete_confirm_container[0]);
